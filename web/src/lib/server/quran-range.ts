...
import { QURAN_CATALOG, QURAN_NAVIGATION, toSurahLink } from "$lib/server/quran-metadata";
...
export function loadRangeData(kind: "juz" | "page", index: number): RangePageData {
  ...
  const entry = QURAN_NAVIGATION.rangeByIndex(rangeKind, index);
  ...
  return {
    ...
    surahs: [...surahNums].map((num) => toSurahLink(QURAN_CATALOG.surahByNum(num)!)),
  };
}
