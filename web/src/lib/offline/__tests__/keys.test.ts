...
describe("normalizeDataKey", () => {
  it("strips only the x-sveltekit-* reserved params", () => {
    expect(
      normalizeDataKey("https://h/app/al-kahf/__data.json?x-sveltekit-invalidated=01"),
    ).toBe("/app/al-kahf/__data.json");
  });

  it("preserves application query params (e.g. ?verse= deep links)", () => {
    expect(
      normalizeDataKey(
        "https://h/app/al-kahf/__data.json?verse=50&x-sveltekit-invalidated=01",
      ),
  ...
  it("keeps unrelated params that merely start with x", () => {
    expect(normalizeDataKey("https://h/p?xfoo=1&x-sveltekit-invalidated=0")).toBe(
      "/p?xfoo=1",
    );
  });
  ...
  it("returns the bare pathname when every param is reserved", () => {
    expect(
      normalizeDataKey(
        "https://h/p?x-sveltekit-invalidated=0_1&x-sveltekit-trailing=1",
      ),
    ).toBe("/p");
  });
  ...
  it("accepts a relative path (the pack's path-only route keys)", () => {
    expect(normalizeDataKey("/app/al-kahf/__data.json")).toBe(
