...
A second pass re-checked all 13 findings against current (2026) primary sources — MDN (prose + Browser-compatibility/Specifications tables), Can I Use, the WHATWG/W3C specs, and browser release notes / bug trackers — with an independent adversarial reviewer per finding. **Net: the audit holds and is sharpened.** Both mediums (M1, M2) and the eight previously-UNCERTAIN low/info findings (L2–L9, I1) are re-confirmed — their load-bearing platform premises all verify against primary sources. **Two findings are refuted (L1, L10);** each rested on a platform premise that is demonstrably false. All stage-1↔stage-2 cross-checks agreed; L10's skeptic pass hit a provider rate-limit (429) and was adjudicated manually against the spec (noted at L10).

> Process note on L1: the verifier's refutation cited `sveltejs/kit#7778`, a GitHub issue whose page contains no resolution and does **not** support the claim. That citation was re-checked by hand against the **installed** SvelteKit source; the conclusion still holds, via the real evidence path given at L1 below. (Lesson recorded: a weak source can still accompany a correct conclusion — verify the conclusion, don't trust the link.)
...
- **L1. `responseSize` returns 0, disabling the byte budget** — FINAL **info (REFUTED, was low/CONFIRMED)**. `__data.json` is never SW-cached: SvelteKit **2.70.1** sets `cache-control: private, no-store` on every `__data.json` path (`node_modules/@sveltejs/kit/src/runtime/server/data/index.js:149` and `:173` — streaming and non-streaming — **verified in the installed tree**); `applyHeaders` preserves it (`hasNoStore` branch → `no-store`); `isCacheable` rejects it; so `handleData` never reaches `data.put`/`recordDataEntry` and `eq-data-v1` stays empty. The byte-budget gap is therefore unreachable. `https://fs.spec.whatwg.org/` is N/A here; the governing evidence is the installed Kit source above.
...
### Larger fact surfaced by the L1 refutation

Because SvelteKit stamps `private, no-store` on `__data.json`, the service worker never caches it — so the **entire `eq-data-v1` SWR + data-meta subsystem (`handleData`, `recordDataEntry`, `enforceDataBounds`, `DATA_MAX`/`DATA_BUDGET_BYTES`, the serialized-meta lock) is inert in production** for its documented input, despite `docs/quran-system.md:93` describing `eq-data-v1` as a live bounded cache. The code is correct and unit-tested against synthetic cacheable responses (`service-worker-data-cache.test.ts`), but no stock `__data.json` ever reaches it. **Suggested team follow-up:** confirm intent — keep it as defense-in-depth for a future route that emits a cacheable `__data.json`, or retire the subsystem. (Note: if any route overrides SvelteKit's header to a cacheable value, `eq-data-v1` re-activates and L1 re-applies — neither case was exhaustively ruled out across every route.)

### Still genuinely unverifiable from static + web analysis (needs runtime)

M2's real-world race incidence under stacked-translation navigation; L6's self-healing on tab freeze/discard across browsers; L8's HTTP-cache softening of the second `downloadBytes` (depends on the immutable Quran-DB Cache-Control); and whether any production route overrides SvelteKit's `__data.json` header (which would re-activate `eq-data-v1` and L1 with it).
