...
One rule set, two implementations (Rust + web Worker) returning identical ordered results. Both strip the full Quranic-sign range `U+06D6–U+06ED` (contiguous) plus `U+064B–U+0658`, `U+0670`, `U+0640`: Rust lists the ranges explicitly (`normalize.rs`); the web's `REMOVED` class covers most of it via `\p{Mn} \p{Me} \p{Cf}` and lists the non-`Mn`/`Me`/`Cf` members — `U+0640`, `U+06DE`, `U+06E5` (`Lm`), `U+06E6` (`Lm`), `U+06E9` (`So`) — explicitly, since `\p{Mn}` alone would miss them (`U+06E5`/`U+06E6` appear in 990/806 verses, `U+06E9` in 15, in Uthmani; 0 in simple-clean). The web also strips `U+0659–U+065F` (`Mn`) where Rust's explicit `064B–0658` range stops at `0658`, but those occur **0× in both corpora** — a harmless no-op asymmetry. A shared neutral parity corpus enforces the contract — see §8.

- **normalize** (`quran/normalize.rs` ↔ web): drop combining marks U+064B–U+0658, U+0670 (superscript alef — dropped, not folded, to match the web's `/\p{Mn}/u`), U+0640 (tatweel), and Quranic signs **U+06D6–U+06DC, U+06DE, U+06DF–U+06E8, U+06E9, U+06EA–U+06ED** (the web lists `U+06E5`/`U+06E6`/`U+06E9` explicitly since they are `Lm`/`So`, not `Mn`). Fold `آ أ إ ٱ → ا`, `ى → ي`, `ة → ه`.
  The range looks continuous but isn't fully: **U+06DD never occurs** in either corpus (no-op whether stripped or not), while **U+06DE** (rub-el-hizb / ruku marker) occurs **199× in Uthmani**, 0× in simple-clean, and **is now stripped** here — see §9. This strip is **search/normalize-layer only**, to keep the highlight offset-map aligned; the **display layer never normalizes**, so `۞` renders verbatim in the reader — matching quran.com / myislam.org.
...
- **U+06DE (`۞`, rub-el-hizb / ruku marker) — resolved.** 199 ayahs of Uthmani, 0 in simple-clean. The **display layer renders it verbatim** (the reader never normalizes → matches quran.com / myislam.org); the **search-highlight layer** now strips it in both Rust (`normalize.rs`; the highlight call at `controller.rs:1141` passes `view.text`) and web (`normalize.ts` — `/\p{Mn}/u` missed it, category `So`, so the codepoint is now listed explicitly), and a case was added to the §8 parity corpus. U+06DD needs no handling — 0 occurrences in either corpus.
