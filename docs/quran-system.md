...
Single source of truth for the shipped system: Quran data, API, delivery, caching, search normalization, UI i18n, web auth, and Rust request security. **Implemented-system index** — every claim is verified against code; implementation detail lives in code, not here. Condensed. (The full per-rule quran.com probe evidence and dated snapshots from the superseded `quran.md`, `caching-architecture.md`, and `quran-normalization-reasoning.md` remain in git history.)
...
- **Budgets.** `web/scripts/audit-i18n-chunks.ts` + `web/i18n-budgets.json`, wired into `postbuild` as `pnpm i18n:budget` (message counts budgeted exactly; gzip gets 2% headroom because chunk hashes move sizes by a byte or two; `--update` re-seeds, `--report` never fails). A rise in one page's budget means it started paying for copy it does not render, so raising a number needs a reason in the PR. Results after the split (measured, worst page per surface): chrome floor **38 messages**; `about` 56 msgs / 6.8 KB gz, `faq` 58 / 11.2, `contact` 49 / 5.8, `privacy` 64 / 7.8, `terms` 63 / 7.3, landing 69 / 8.4, reader pages 155 / 7.8. The decisive check: each page's rise equals its own namespace size exactly, so no page pays for another's copy — 96 messages were added for the remaining marketing pages without moving any existing page's budget.
...
- **How to add a page:** add `<page>_*` keys to both catalogs; claim the prefix in `i18n-namespaces.json` (an unclaimed key fails generation); `pnpm i18n:check` regenerates the barrel; write `<page>-copy.ts` importing only that barrel; add a budget entry and run `pnpm build` (the audit prints the page's cost). Anything that moves another page's budget fails the audit.
...
7. **Artifact safety is the executable contract.** Production download specs are reconstructed from baked `{id, sizeBytes, r2Path, sameOriginDeliveryPath}` maps; remote catalogue metadata may describe availability but never selects Quran bytes, size, or a delivery origin, and `eq-data-v1` metadata is bounded the same way. Downloads stage into an id-scoped temp file, then validate exact baked size plus content invariants (6,236 contiguous rows, tiling, ayah keys, packaging — content asserts, never hashes) before an IDB `{sourceId, activeFile}` pointer switches atomically and the prior file is removed. A partially written DB can never become active; invalid legacy files stay inactive and trigger normal redownload.
...
- Paraglide 2.23 ships every locale's strings to every visitor (all locales inlined per message module); accepted while locale count is low — upstream per-locale splitting is open. Costs are contained by namespace barrels + the postbuild budget guard (see "Message chunking and budgets" in Part 2).
