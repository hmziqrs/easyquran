...
Single source of truth for the shipped system: Quran data, API, delivery, caching, search normalization, UI i18n, web auth, and Rust request security. **Implemented-system index** — every claim is verified against code; implementation detail lives in code, not here. Condensed. Supersedes the former `quran.md`, `caching-architecture.md`, `quran-normalization-reasoning.md`, `i18n.md`, and `implementation-plan.md` (all shipped — folded in below; the full per-rule quran.com probe evidence and dated snapshots remain in git history).
...
- **Service worker:** four Cache-Storage buckets (app shell, pages LRU, `__data.json` SWR, offline pack) + IDB meta; atomic install/activate/handoff lifecycle; contract shared via `./lib/offline/messages` (only `META_DB`/`META_STORE` + `normalizeDataKey` genuinely duplicated). Same-origin `/api/` requests **never** enter any Cache-Storage bucket (unconditional bypass). `eq-data-v1` is bounded by count (`DATA_MAX` 400) and bytes (32 MiB) with per-key `{key, lastUsed, sizeBytes}` metadata, LRU eviction, and startup reconciliation; every deletion path removes metadata.
...
### Baked artifact delivery (W10a)

- Authoritative baked maps `{id, sizeBytes, r2Path, sameOriginDeliveryPath}` are built at compile time; remote catalogue metadata may describe availability but never selects Quran bytes, size, or a delivery origin. API artifact URLs are parsed as HTTPS with no credentials/query/fragment; their canonical R2 pathname and `sizeBytes` must equal the baked values. Download URL + size are constructed **only** from baked fields. Unknown id / size-path mismatch / malformed payload falls back to baked data and emits consent-gated browser telemetry. Arabic script decoding applies the same exact baked-size + canonical-R2-path checks.

### Crash-safe OPFS replacement (W10c)

- `sizeBytes` is required at every production `DownloadSpec` boundary (tests may build invalid specs only via an explicit unsafe fixture). Downloads stage into an id-scoped temp file, close it, verify exact baked size, then open the staged SQLite in the worker and assert schema + 6,236 contiguous rows + source id (content asserts, never hashes) before the IDB `{sourceId, activeFile}` pointer switches in one transaction and the old file is removed. Failure keeps/reverts to the last validated active file and cleans temp files. A partially written DB can never become active. On first pointer-aware boot a valid legacy `<id>.sqlite` adopts as active; invalid legacy files and abandoned temps never appear as sources.

### Reader fallback ladder + range client data (W5/W6/W7)

- **One ladder for both source kinds:** `local → API → local re-check → typed failure` (`withSourceFallback`). Worker readiness is awaited within `LOCAL_BOOT_BUDGET_MS` before choosing API; no worker-status gate may block a read. Tier outcomes are typed (`servedBy` + `workerFailure`/`apiFailure`), so worker and API degradation stay independently observable.
...
- **Range routes upgrade after paint.** `RangeReader` installs the server snapshot, then starts a client read keyed by `{sourceId, kind, index}`; the result installs only if the route key still matches, so late results cannot cross route or source boundaries. A complete first paint skips the duplicate read; degraded paints retry. Reader links keep translation context through the `surah*For`/`juzPathFor`/`globalPagePathFor` family.
...
- **Boot metadata is baked-first.** `/scripts` + `/sources` start from the baked manifest/catalogue, then refresh validated metadata asynchronously after worker start; a metadata timeout or failure keeps baked state without delaying Arabic boot, and remote manifest data never controls delivery fields.

### Engagement-gated translation prefetch (W4)
...
- Generated `src/lib/paraglide/` is gitignored build artefact. `pnpm i18n:check` (catalog parity check + compile + namespace codegen) runs in `precheck`/`prelint`/`pretest`/`prebuild`; `pnpm i18n:budget` runs in `postbuild` and enforces `web/i18n-budgets.json`.
- `web/src/lib/i18n/`: `locales.ts` (`UI_LOCALES` registry — `en`/ltr/`en_US`, `ar`/rtl/`ar_SA`; the registry, not the Quran catalogue, drives switchers, sitemap fan-out, hreflang, prerender discovery), `marketing.ts` (`MARKETING_PUBLICATIONS` matrix — only `home` publishes `ar` today; `marketingHref` returns `null` for unpublished pairs), `reader.ts` (`readerHrefFor`/`readerHomeHrefFor` — the **only** allowed reader UI-prefix operation; accepts only a supported `UiLocale` + an origin-relative canonical `/app` href), copy resolvers (`chrome-copy.ts`, `appearance-copy.ts`, `landing-copy.ts`, `reader-copy.ts` with `ReaderUiCopy`), and generated per-namespace barrels in `src/lib/i18n/m/**` (auto-generated by `scripts/gen-message-namespaces.ts`; the global message barrel is banned — `message-barrel-guard.test.ts`). Per-page chunking, the 38-message chrome floor, and budget mechanics are owned by [`./i18n-bundle-plan.md`](./i18n-bundle-plan.md).

## Routing and prerendering

- `web/src/app.html` carries `<html lang="%lang%" dir="%dir%">`; one composed root-HTML transform replaces both placeholders on **every** document (marketing overrides both from `UiLocale`; reader routes keep source-document language/direction; no placeholder may survive an emitted or cached document).
...
`ui` is `en` or `ar`; `contentLang`/`translator` remain the immutable translation-source selector — reader UI locale changes no source selection. Route components, loaders, and Rust calls receive the existing source context, not a `UiLocale` route parameter.
...
# Part 3 — Web auth and cache isolation (shipped)
...
## Rate limiting + ban escalation (W3)

- Fixed windows: 600/60s general `quran-v1`, 30/60s search (double-limited), 600/min internal SSR, 120/min readiness. The store prunes expired **claims** alongside limits (W10b) so dedup claims cannot accumulate without bound.
- **Escalation is default-off** (`QURAN_BAN_ESCALATION_ENABLED=false`) and gated on the W2 ingress contract. It never touches `on_block` (a synchronous response hook); it runs in `RateLimitMiddleware::call` with resolved identity + store available.
...
## Translation pool + API-demand prewarm (W1)
...
- Observability: `PoolStats` carries `prewarmed: Vec<String>` + `top_demand: Vec<(String, f64)>` (cap 10); `hit_rate` is `Option<f64>` serialized `null` when `lookups == 0` (the old misleading `1.0` is gone). `/quran/health/ready` reads a ranked snapshot from an `RwLock` — `stats()` never reads SQLite, and no guard is held across `.await`. Boot-candidate OTLP counters distinguish real cold builds for the candidate set from prewarm's own builds.

---

# Part 5 — Divergences from `my-plan-raw.md`

[`docs/my-plan-raw.md`](./my-plan-raw.md) is owner-authored and read-only; it records intent, not the delivery architecture this repository ships. The seven boundaries below are load-bearing and must not be blurred by an implementation or a later doc edit. Recorded here, not back in the raw plan.
...
# Part 6 — Status, known gaps, follow-ons

**Shipped:** artifact URL contract · no-sha identity · translation pool + API-demand prewarm · `/sources` catalogue · `/t` translation reader routes across four route families · OPFS retention + crash-safe staged replacement · production Bun SSR + disk-TTL · normalize-parity fixture · quran.com search parity · UI i18n milestone 1+2 (localized `/` + `/ar/` marketing, `/en/app/**` + `/ar/app/**` reader URLs, localized SEO/sitemap, RTL base) · per-page message chunking + budgets · engagement-gated translation prefetch · unified fallback ladder + exact range chunking · range client data path + bounded `eq-data-v1` · pending-response recovery without document reloads · web auth (email/password, OAuth × 4, passkeys, sessions, cache isolation) · typed request identities + health isolation · ban escalation/persistence/admin/export · bounded dedup claims.
...
- Paraglide 2.23 ships every locale's strings to every visitor (all locales inlined per message module); accepted while locale count is low — upstream per-locale splitting is open. Costs are contained by namespace barrels + the postbuild budget guard (see [`./i18n-bundle-plan.md`](./i18n-bundle-plan.md)).
...
- W1's `QURAN_DEMAND_COLLECT` / `QURAN_PREWARM_TRANSLATIONS` are parsed in `main.rs` rather than `config/settings.rs` (defaults and semantics as specified — cosmetic deviation).
