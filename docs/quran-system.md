...
6. **Authentication is client-hydrated everywhere.** Arabic reader and account shells are prerendered and auth-neutral; the four translated reader route families remain auth-neutral SSR. User state enters neither the build output, the translated HTML disk cache, nor Cache Storage (W8) — no per-user content in any shared cache.
